...
> 번역 키를 타입으로 잠그려고, 나는 먼저 "코드를 훑어 `.d.ts`를 뽑아주는 생성기"부터 떠올렸다. 그런데 막상 해보니 내가 직접 짠 건 열 줄 남짓이었다.
> 중첩 키 평탄화처럼 어려운 부분은, 이미 i18next 타입이 다 해두고 있었기 때문이다.

> #2에서 "번역 함수의 `key`를 유효 키 집합으로 좁히면 오타·미정의·네임스페이스 오용이 컴파일 에러가 된다"를 봤다. 이번 글은 **그 타입을 실제로 만드는 방법**이다.
